...
void Chassis::inlineTelOp(int leftY, int rightX){// 127, -127
    float forward = 0;//4.7244 * leftY
    float turn = 0;//4.7244 * rightX

    if(leftY > 10 || leftY < -10){ // if statement to implement deadzone
        forward = 4.7244 * leftY;
    }
...
double Chassis::getDriveEncoder(){
    return (fabs(frontLeft.get_position()) +fabs(frontRight.get_position()) + fabs(backLeft.get_position()) + fabs(backRight.get_position()))/4;
}
...
void Chassis::moveFowardMotor(double target, int speed){
    int counter = 0;
    resetMotorEncoders();
    ...
    while(counter < 3){
        if(target - getDriveEncoder() > target / 10 && target > 0){
            moveSteering(-keepStraight.compute(inertial.get_rotation()), speed);
        }
        else if(fabs(target) - getDriveEncoder() > fabs(target) / 10 && target < 0){
            moveSteering(keepStraight.compute(inertial.get_rotation()), speed);
        }
        else{
            move(forwardController.compute(getDriveEncoder()), forwardController.compute(getDriveEncoder()));
        }
        if(getDriveEncoder() > target-2 && getDriveEncoder()< target + 2){
            counter++;
        }
        else{
            counter = 0;
